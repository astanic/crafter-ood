...
class Env(BaseClass):
    ...
    def __init__(self, area=(64, 64), view=(9, 9), size=(64, 64), reward=True, length=10_000, seed=None, 
            render_scoreboard=True, double_render=False, render_to_disc=False, el_vars='', el_freq='100,0,0,0', total_dreamer=False, el_app_freq=None):
        ...
        self._episode = 0
        self._step = 0
        self._world = engine.World(area, constants.materials, (12, 12), el_vars, el_freq, total_dreamer, el_app_freq)
        self._textures = engine.Textures(constants.root / 'assets', self._world)
        item_rows = int(np.ceil(len(constants.items) / view[0]))
        self._local_view = engine.LocalView(self._world, self._textures, [view[0], view[1] - item_rows])
        self._item_view = engine.ItemView(self._textures, [view[0], item_rows], render_scoreboard=render_scoreboard)
    ...
    def step(self, action):
        ...
        if self._double_render:
            info['obs72'] =  self.render(size=(72, 72))
        if not self._reward:
            reward = 0.0
    ...
    def render_to_disk(self, size=None):
        ...
        obs = canvas.transpose((1, 0, 2))
        img1 = obs

        from PIL import Image
        img1 = Image.fromarray(img1)
        img1.save(f"img{self._step:02d}.jpeg")
        self._step += 1
        if self._step == 200:
            exit(0)
    ...
    def _update_time(self):
        # https://desmos.com/calculator/grfbs6rs3h
