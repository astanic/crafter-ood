...
class Object:
    ...
    @property
    def texture_vars(self):
        if hasattr(self, 'variant_name'):
            return self.texture + self.variant_name
        else:
            return self.texture
    ...
    def is_free(self, target, materials=None):
        ...
        material, obj, _ = self.world[target]
...
class Player(Object):
    ...
    def __init__(self, world, pos):
        ...
        self._fatigue = 0
        self._recover = 0
        self.variant_name = world.get_el_var(self.texture)
    ...
    def update(self):
        ...
        for name, amount in self.inventory.items():
            maximum = constants.items[name]['max']
            self.inventory[name] = max(0, min(amount, maximum))
