...
class BaseModel(nn.Module, ABC):
    ...
    def _update_features_extractor(
        ...
        if features_extractor is None:
            ...
            features_extractor = self.make_features_extractor(args)
        net_kwargs.update(dict(features_extractor=features_extractor, features_dim=features_extractor.features_dim))
        return net_kwargs

    def make_features_extractor(self, args) -> BaseFeaturesExtractor:
        """Helper method to create a features extractor."""
        return self.features_extractor_class(args, self.observation_space, **self.features_extractor_kwargs)
    ...
    def extract_features(self, obs: th.Tensor) -> th.Tensor:
        ...
        features, attn_maps = self.features_extractor(preprocessed_obs)
...
class BasePolicy(BaseModel):
    ...
    def predict(
        ...
        with th.no_grad():
            actions, attn_maps = self._predict(observation, deterministic=deterministic)
        # Convert to numpy
        ...
        return actions, state, attn_maps
...
class ActorCriticPolicy(BasePolicy):
    ...
    def __init__(
        ...
        self.args = args
        self.fe_type = args.fe.type
        if args.fe.type == 'flatten':
            features_extractor_class = FlattenExtractor
        elif args.fe.type == 'cnn':
            features_extractor_class = NatureCNN
        elif args.fe.type == 'i2s':
            features_extractor_class = I2SFeatureExtractor
        elif args.fe.type == 'multi-input':
        ...
        if net_arch is None:
            if features_extractor_class in (NatureCNN, I2SFeatureExtractor):
                net_arch = []
            else:
        ...
        self.features_extractor = features_extractor_class(args, self.observation_space, action_space, **self.features_extractor_kwargs)
    ...
    def _build_mlp_extractor(self) -> None:
        ...
        self.pi_vf_extractor = MlpExtractor(
            self.features_dim,
            net_arch=self.net_arch,
            activation_fn=self.activation_fn,
            device=self.device,
    ...
    def _build(self, lr_schedule: Schedule) -> None:
        ...
        latent_dim_pi = self.pi_vf_extractor.latent_dim_pi
        ...
        self.value_net = nn.Linear(self.pi_vf_extractor.latent_dim_vf, 1)
        ...
        if self.ortho_init:
            ...
            module_gains = {
                ...
                self.pi_vf_extractor: np.sqrt(2),
    ...
    def forward(self, obs: th.Tensor, deterministic: bool = False) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        ...
        features, attn_maps = self.extract_features(obs)
        latent_pi, latent_vf = self.pi_vf_extractor(features)
    ...
    def _get_latent(self, obs: th.Tensor, deterministic: bool = False) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        features, attn_maps = self.extract_features(obs)
        latent_pi, latent_vf = self.pi_vf_extractor(features)
    ...
    def _predict(self, observation: th.Tensor, deterministic: bool = False) -> th.Tensor:
        ...
        :return: Taken action according to the policy
        """
        distribution, attn_maps = self.get_distribution(observation)
        return distribution.get_actions(deterministic=deterministic), attn_maps
    ...
    def get_distribution(self, obs: th.Tensor) -> Distribution:
        ...
        """
        features, attn_maps = self.extract_features(obs)
        latent_pi = self.pi_vf_extractor.forward_actor(features)
        return self._get_action_dist_from_latent(latent_pi), attn_maps
    ...
    def predict_values(self, obs: th.Tensor) -> th.Tensor:
        ...
        """
        features, attn_maps = self.extract_features(obs)
        latent_vf = self.pi_vf_extractor.forward_critic(features)
